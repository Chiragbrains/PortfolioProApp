-- Fix embedding column name and ensure proper vector dimension

-- First, drop any existing indexes that might be using the wrong column name
DROP INDEX IF EXISTS portfolio_summary_embedding_idx;

-- Rename the column if it exists as 'embedded'
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 
